...
async function createTask(req, res) {
    ...
    try {
        ...
        await (0, logger_1.logActivity)('task_created', `Task "${task.title}" created in project "${project.name}"`, authReq.user?.id, tenantId);
...
async function getTasks(req, res) {
    ...
    const { projectId, status, priority, assignedToId, search, sortBy = 'createdAt', sortOrder = 'desc', page = '1', limit = '100', } = req.query;
    ...
    try {
        const totalCount = await db_1.prisma.task.count({ where: whereClause });
...
async function updateTask(req, res) {
    ...
    try {
        ...
        const existingTask = await db_1.prisma.task.findFirst({
            where: { id, tenantId },
        });
        if (!existingTask) {
        ...
        await (0, logger_1.logActivity)('task_updated', `Task "${updatedTask.title}" was updated (status: ${updatedTask.status})`, authReq.user?.id, tenantId);
        // Emit Real-time WebSocket update
        const io = req.app.get('io');
        if (io) {
...
async function deleteTask(req, res) {
    ...
    try {
        ...
        await db_1.prisma.task.delete({
            where: { id },
        });
        await (0, logger_1.logActivity)('task_deleted', `Task "${task.title}" was deleted`, authReq.user?.id, tenantId);
        // Emit Real-time WebSocket update
        const io = req.app.get('io');
        if (io) {
